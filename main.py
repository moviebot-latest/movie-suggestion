# ╔══════════════════════════════════════════════════════════════════════════╗
# ║         🎬  CineBot v9 — FULLY MERGED (v8 + v9 patch + movie_info)     ║
# ║                                                                          ║
# ║  ✅ V9 CHANGES:                                                         ║
# ║     • All v8 bugs fixed                                                  ║
# ║     • W_ADDADMIN conversation state added                                ║
# ║     • Admin panel animated opening + live stats dashboard                ║
# ║     • Add Admin from panel (conversation flow)                           ║
# ║     • Remove Admin from panel (button-based)                             ║
# ║     • Admin List with live expiry countdown                              ║
# ║     • double query.answer() bugs fixed                                   ║
# ║     • Version string v6→v9 fixed everywhere                             ║
# ║                                                                          ║
# ║  ✅ movie_info MODULE INTEGRATED:                                        ║
# ║     • TMDB → full movie details (genres, runtime, cast, director)       ║
# ║     • OMDB → poster (primary) + TMDB poster fallback                    ║
# ║     • Auto retry (3x), timeout protection, in-memory cache              ║
# ║     • /movieinfo command — rich movie card via new module                ║
# ║                                                                          ║
# ║  APIs needed:                                                            ║
# ║     BOT_TOKEN, OMDB_API, TMDB_API, GEMINI_API, ADMIN_ID                ║
# ╚══════════════════════════════════════════════════════════════════════════╝
...
# movie_info module also uses these keys
...
@web_app.route("/")
def home(): return "🎬 CineBot v9 Running"
...
# ═══════════════════════════════════════════════════════════════════
#         ✅ MOVIE INFO MODULE (integrated from movie_info.py)
# ═══════════════════════════════════════════════════════════════════
...
async def get_movie_info(title: str) -> Optional[dict]:
    """
    Movie ki complete info fetch karo (TMDB + OMDB).
    Returns dict with: title, year, genres, runtime, rating, votes,
    overview, poster, imdb_id, tmdb_id, director, cast, tagline, language, budget, revenue
    """
...
async def send_movie_card(
    ...
    """
    movie_info module ka Telegram sender.
    /movieinfo command ke liye use hota hai.
    Rich card: TMDB genres, runtime, cast, director, budget, revenue.
    """
...
# ═══════════════════════════════════════════════════════════════════
#                   ✅ MOVIE CARD (OMDB-based, original)
# ═══════════════════════════════════════════════════════════════════
...
# ✅ NEW: /movieinfo command — uses movie_info module (TMDB-rich)
async def movieinfo_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /movieinfo Inception
    TMDB se full details: genres, runtime, cast, director, budget, revenue
    """
    ...
    if not TMDB_API:
        await update.message.reply_text(
            "⚠️ *TMDB_API not set!*\n\n_Set TMDB_API in environment variables._",
            parse_mode="Markdown"
        )
        return
...
async def compare_recv2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result:
        ...
        await update.message.reply_text("❌ Comparison nahi hua. GEMINI_API check karo.", parse_mode="Markdown")
    return ConversationHandler.END
...
async def clean_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("❌ *Cannot delete.*\n\n_Make bot admin in group to delete messages._", parse_mode="Markdown")


# ═══════════════════════════════════════════════════════════════════
#   MULTI-ADMIN MANAGEMENT  ✅ v9 (all bugs fixed)
# ═══════════════════════════════════════════════════════════════════
        ...
        try:
            ...
            try:
                await context.bot.send_message(chat_id=target_id, text=(
                    ...
                    f"⏱ *Duration:* `{hours} ghante`\n📅 *Expires:* `{expiry_str}`\n\n🔑 *Admin Panel:* /admin\n\n_Apni powers responsibly use karo!_ 🎬"
                ), parse_mode="Markdown")
...
# ✅ FIX #4 v9 — double query.answer() fixed + W_ADDADMIN flow
async def adm_addadmin_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sent = await query.message.reply_text(
        ...
        "*Examples:*\n`987654321` — Permanent\n`987654321 24` — 24 ghante ke liye\n\n"
        "ℹ️ _User ID pata nahi? Unhe /start karwao._\n\n❌ /cancel — Cancel karo",
        parse_mode="Markdown"
    )
...
# ✅ NEW v9: Receive admin ID from panel conversation
async def adm_addadmin_recv(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            "❌ *Invalid format!*\n\nSirf User ID numbers mein likhein.\nExample: `987654321` ya `987654321 24`\n\n/cancel",
            parse_mode="Markdown"
        )
        return W_ADDADMIN
    ...
    if len(parts) >= 2:
        try:
            ...
            await update.message.reply_text(
                "❌ Ghante galat hain! Correct format: `USER_ID GHANTE`\nExample: `123456 24`\n\n/cancel",
                parse_mode="Markdown"
            )
            return W_ADDADMIN
    else:
        ...
        sent = await update.message.reply_text(
            f"╔══════════════════════════╗\n║  ✅  *ADMIN ADDED!*  ║\n╚══════════════════════════╝\n\n"
            f"👤 User ID: `{target_id}`\n🔑 Type: `Permanent`\n\n_/removeadmin {target_id} se hata sakte ho_\n_/admin — Panel par wapas jao_",
            parse_mode="Markdown"
        )
        asyncio.create_task(auto_delete(sent, 60))
        try:
...
# ✅ FIX #5 v9 — double query.answer() fixed
async def adm_listadmins_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not admins:
        sent = await query.message.reply_text(
            f"╔══════════════════════╗\n║  👑  *ADMIN LIST*  ║\n╚══════════════════════╝\n\n"
            f"_Koi extra admin nahi hai._\n\n👑 Owner: `{ADMIN_ID}` _(ENV — permanent)_\n\n➕ Add karne ke liye: /addadmin USER_ID",
            parse_mode="Markdown"
        )
        asyncio.create_task(auto_delete(sent, 60))
        return
...
# ✅ NEW v9: Remove admin from panel button
async def adm_rmadmin_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if target_id in admins:
        ...
        await query.message.edit_text(f"⚠️ User `{target_id}` admin list mein nahi tha.", parse_mode="Markdown")


# ═══════════════════════════════════════════════════════════════════
#   ADMIN PANEL  ✅ v9 (animated + live stats + banned count)
# ═══════════════════════════════════════════════════════════════════
...
# ✅ FIX #3 v9 — adm_back v6→v9 + banned count + admins count
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "📋 *Commands:*\n"
        "🎬 /movieinfo — TMDB rich movie info (NEW)\n"
...
# ✅ master_conv v9 — W_ADDADMIN state + adm_addadmin_cb entry_point
master_conv = ConversationHandler(
    entry_points=[
        ...
        CallbackQueryHandler(adm_addadmin_cb,      pattern="^adm_addadmin$"),  # ✅ NEW v9
    ...
    states={
        ...
        W_ADDADMIN:    [MessageHandler(filters.TEXT & ~filters.COMMAND, adm_addadmin_recv)],  # ✅ NEW v9
    },
...
application.add_handler(CommandHandler("movieinfo",   movieinfo_cmd))   # ✅ NEW: movie_info module
...
application.add_handler(CallbackQueryHandler(adm_rmadmin_cb,    pattern="^adm_rmadmin_"))  # ✅ NEW v9
# NOTE: adm_addadmin_cb is in master_conv entry_points — standalone register NAHI karna

# User callbacks
application.add_handler(master_conv)  # ConversationHandler pehle
...
print("✅ CineBot v9 Running — All Bugs Fixed + Advanced Admin + movie_info Module!")
