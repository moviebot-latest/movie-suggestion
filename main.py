...
# ═══════════════════════════════════════════════════════════════════
#   🔥 SERVER HEALTH CHECKER — v3 ULTRA (fully integrated)
# ═══════════════════════════════════════════════════════════════════
...
async def srv_check_single(key: str, name: str, url: str) -> dict:
    ...
    # ✅ FIX: ?s= wala part hatao, sirf base URL check karo
...
async def srv_check_all_parallel(servers: dict) -> dict:
    """All servers PARALLEL — ~10 sec. Merges uptime history."""
    tasks = [
    ...
    for r in results_list:
        ...
        results[key] = r
        icon = "✅" if r["up"] else "❌"
        print(f"{icon} {r['name']} | {r.get('response_ms')}ms | code={r.get('code')}")
...
# ── Auto 12hr background checker ──
async def auto_server_checker(bot, admin_id: int):
    """
    Har 12 ghante auto check.
    DOWN = admin ko Telegram alert (with Groq AI diagnosis).
    All UP = silent (no spam).
    """
    print("🕐 Auto server checker started (12hr interval)")
    await asyncio.sleep(300)   # 5 min warmup after bot start
    ...
    while True:
        try:
            ...
            if down_keys:
                ...
                print(f"🚨 Alert sent: {len(down_keys)} DOWN")
            else:
