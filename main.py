...
# ║                                                                          ║
# ║  ✅ NEW — SERVER HEALTH CHECKER v3 ULTRA (fully integrated):          ║
...
# ║     • Thread-safe file writes, retry logic (3x), HEAD+GET fallback    ║
# ║                                                                          ║
# ║  ✅ NEW COMMANDS:                                                       ║
...
# ═══════════════════════════════════════════════════════════════════
#   🔥 SERVER HEALTH CHECKER — v4 PRO (Advanced Upgrade)
#
#   NEW in v4 PRO:
#   ✅ Speed rating: FAST/OK/SLOW/DEAD category per server
#   ✅ Uptime % calculation (last 10 checks)
#   ✅ Consecutive failures counter — kitni baar se down hai
#   ✅ Mobile User-Agent rotate karta hai (bot detection avoid)
#   ✅ Smart alert throttle — same server pe bar bar alert nahi
#   ✅ /serverstats command — detailed uptime % stats
#   ✅ Auto interval smart: agar koi DOWN ho to 1hr mein recheck
#   ✅ History 10 tak badha (zyada accurate uptime %)
#   ✅ Avg response time track per server
# ═══════════════════════════════════════════════════════════════════
SRV_CHECK_INTERVAL_HOURS = 12
SRV_FAST_INTERVAL_HOURS  = 1        # ✅ NEW: jab koi server down ho
SRV_RETRY_COUNT          = 3
SRV_RETRY_DELAY          = 3
SRV_REQUEST_TIMEOUT      = 15
SRV_STATUS_FILE          = "server_status.json"
SRV_HISTORY_MAX          = 10       # ✅ v4: 5 → 10 for better uptime %
SRV_ALERT_COOLDOWN_HRS   = 3        # ✅ NEW: same server alert cooldown
_srv_file_lock           = threading.Lock()
_srv_alerted_at          = {}       # ✅ NEW: {server_key: last_alert_timestamp}

# ✅ v4: Multiple User-Agents rotate karo (bot detection avoid)
SRV_USER_AGENTS = [
...
def _get_srv_headers() -> dict:
    """✅ v4: Random User-Agent rotate karo har check mein."""
    return {
...
# ✅ v4: Speed rating function
def _srv_speed_rating(ms: int) -> str:
    if ms <= 0:    return "💀 DEAD"
...
# ✅ v4: Uptime % calculate karo history se
...
async def _srv_check_once(session: aiohttp.ClientSession, url: str) -> tuple:
    """HEAD first (fast), GET fallback. Returns (is_up, code, ms, method)."""
    for method in ("HEAD", "GET"):
        ...
        try:
            ...
            kw = dict(
                url=url,
                headers=_get_srv_headers(),   # ✅ v4: rotating headers
...
async def srv_check_single(key: str, name: str, url: str) -> dict:
    """Single server — retry SRV_RETRY_COUNT times."""
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        for attempt in range(1, SRV_RETRY_COUNT + 1):
            is_up, code, ms, method = await _srv_check_once(session, check_url)
            if is_up:
                return {
                    "key": key, "name": name, "url": url,
...
async def srv_check_all_parallel(servers: dict) -> dict:
    """All servers PARALLEL. Merges uptime history + v4 stats."""
    tasks = [
        srv_check_single(k, v.get("name", k), v.get("url", ""))
    ...
    for r in results_list:
        ...
        r["speed_rating"]  = _srv_speed_rating(r.get("response_ms", 0)) if r["up"] else "💀 DOWN"
        results[key] = r
        icon = "✅" if r["up"] else "❌"
        print(f"{icon} {r['name']} | {r.get('response_ms')}ms | {r['speed_rating']} | uptime={r['uptime_pct']}")
    srv_save_status(results)
    return results

async def srv_ai_diagnose(results: dict) -> Optional[str]:
    """Groq AI — DOWN servers ka diagnosis."""
    if not GROQ_API:
        return None
    ...
    if not down:
        return None
    down_info = "\n".join(
        f"- {r['name']} | URL: {r['url'][:55]} | Code: {r.get('code',0)} | "
        f"Error: {r.get('method','?')} | Consec fails: {r.get('consec_fails',0)} | "
    ...
    prompt = (
        ...
        f"1. Likely reason (based on code/error/consecutive fails)\n"
    ...
    result = await ai_ask(prompt, max_tokens=600)
    return result
...
def srv_format_status(results: dict, title: str = "🖥️ SERVER STATUS") -> str:
    ...
    for key, r in sorted(results.items()):
        ...
        if up:
            lines.append(f"\n✅ *{name}*  {speed} `{ms}ms`")
            lines.append(f"   Code `{code}` | Avg: `{avg_ms}ms` | Uptime: {uptime}")
            lines.append(f"   History (last 10): {bar}")
        else:
            fails = r.get("consec_fails", 0)
            lines.append(f"\n❌ *{name}* — DOWN! 🔴")
            lines.append(f"   Code `{code}` | {r.get('attempts',0)}x failed | {fails} consec ❌")
            lines.append(f"   Uptime: {uptime} | History: {bar}")
    lines.append("\n━━━━━━━━━━━━━━━━━━━━━━━")
    lines.append(f"🔄 Next auto-check: {SRV_CHECK_INTERVAL_HOURS}hrs mein")
    return "\n".join(lines)

# ✅ v4 NEW: /serverstats — detailed uptime % table
def srv_format_stats(results: dict) -> str:
    ...
    for key, r in sorted(results.items()):
        ...
        fails  = r.get("consec_fails", 0)
        up     = r.get("up", False)
        status = "✅ UP" if up else f"❌ DOWN ({fails} consec)"
        lines.append(f"*{name}*")
        lines.append(f"   {status}  |  Uptime: {uptime}")
        lines.append(f"   Avg Speed: `{avg_ms}ms` {speed}")
        lines.append("")
    lines.append("━━━━━━━━━━━━━━━━━━━━━━━")
    lines.append("_History = last 10 checks_")
    return "\n".join(lines)
...
def srv_format_alert(down_keys: list, results: dict) -> str:
    ...
    for key in down_keys:
        ...
        lines += [
            f"🔴 *{r.get('name', key)}*",
            f"   `{r.get('url','')[:55]}`",
            f"   Code `{r.get('code',0)}` | {r.get('attempts',0)}x failed | {fails} consec ❌",
            f"   Uptime: {uptime} | History: {_srv_history_bar(r.get('history',[]))}",
            "",
        ]
    lines += [
        "━━━━━━━━━━━━━━━━━━",
...
async def checkservers_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    loading = await update.message.reply_text(
        ...
        "🔄 User-Agent rotating (anti-bot bypass)\n"
    ...
    ai_text = None
    if down_keys:
        await loading.edit_text("🤖 *AI DOWN server analyze kar raha hai...*", parse_mode="Markdown")
        ai_text = await srv_ai_diagnose(results)

    text = srv_format_status(results, "🖥️ SERVER CHECK v4 PRO")
    if ai_text:
        text += f"\n\n🤖 *AI DIAGNOSIS:*\n{ai_text}"
...
async def srvchk_refresh_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    results   = await srv_check_all_parallel(servers)
    down_keys = [k for k, v in results.items() if not v.get("up")]
    ai_text   = await srv_ai_diagnose(results) if down_keys else None
    text      = srv_format_status(results, "🔄 SERVER REFRESH v4 PRO")
    if ai_text:
        text += f"\n\n🤖 *AI DIAGNOSIS:*\n{ai_text}"
    kb = [
...
# ── Admin panel "📡 Server Status" button callback ──
...
# ── Auto background checker — v4: smart interval ──
async def auto_server_checker(bot, admin_id: int):
    """
    v4 PRO:
    - All UP → 12hr interval (silent)
    - Any DOWN → 1hr fast recheck
    - Alert throttle: same server pe {SRV_ALERT_COOLDOWN_HRS}hr mein ek hi alert
    """
    print("🕐 Auto server checker v4 PRO started")
    await asyncio.sleep(300)   # 5 min warmup

    while True:
        try:
            print(f"🔍 Auto server check @ {datetime.now().strftime('%H:%M')}")
            servers   = load_servers()
            results   = await srv_check_all_parallel(servers)
            down_keys = [k for k, v in results.items() if not v.get("up")]

            if down_keys:
                # ✅ v4: Alert throttle — already alerted recently? skip
                now_ts    = time.time()
                to_alert  = []
                for k in down_keys:
                ...
                if to_alert:
                    ai_text = await srv_ai_diagnose(results)
                    alert   = srv_format_alert(to_alert, results)
                    if ai_text:
                        alert += f"\n\n🤖 *AI DIAGNOSIS:*\n{ai_text}"
                    kb = [[
                    ...
                    print(f"🚨 Alert sent: {len(to_alert)} servers (throttled {len(down_keys)-len(to_alert)})")
                else:
                    print(f"⏳ {len(down_keys)} down but alert throttled (cooldown active)")

                # ✅ v4: Fast recheck mode jab servers down ho
                sleep_hrs = SRV_FAST_INTERVAL_HOURS
                print(f"⚡ Fast mode: recheck in {sleep_hrs}hr (servers down)")
            else:
                # ✅ Reset alert cooldown jab server wapas UP ho
                for k in list(_srv_alerted_at.keys()):
                    if results.get(k, {}).get("up"):
                        _srv_alerted_at.pop(k, None)
                print("✅ All servers UP — no alert sent")
                sleep_hrs = SRV_CHECK_INTERVAL_HOURS

        except Exception as e:
            print(f"Auto server checker error: {e}")
            sleep_hrs = SRV_CHECK_INTERVAL_HOURS

        nxt = datetime.now() + timedelta(hours=sleep_hrs)
        print(f"⏰ Next check: {nxt.strftime('%Y-%m-%d %H:%M')} ({sleep_hrs}hr)")
        await asyncio.sleep(sleep_hrs * 3600)
# ═══════════════════════════════════════════════════════════════════
#   END SERVER HEALTH CHECKER v4 PRO
# ═══════════════════════════════════════════════════════════════════
