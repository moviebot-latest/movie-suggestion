# ╔══════════════════════════════════════════════════════════════════════╗
# ║            🎬 CineBot v6 — OMDB FRESH FETCH + SAFETY FIX            ║
# ║                                                                      ║
# ║  ✅ V6 CHANGES:
# ║     • review_cb: OMDB se fresh fetch (session expire fix)
# ║     • funfact_cb: OMDB se fresh fetch (session expire fix)
# ║     • Buttons: rev_{imdb_id} & fun_{imdb_id} (msg_id hataya)
# ║     • ai_ask: BLOCK_NONE safety filters (Gemini block fix)
# ║
# ║  🐛 ALL BUGS FIXED:                                                 ║
...
async def review_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    movie_data = get_omdb(imdb_id, by_id=True)
    ...
    # Gemini ko details bhejein
    review = await ai_movie_review(
        movie_data["Title"], movie_data["Year"],
        movie_data["Plot"], movie_data["imdbRating"]
    )
    ...
    if review:
        await query.message.reply_text(f"🎬 *{movie_data['Title']}* Review:\n\n{review}", parse_mode="Markdown")
    else:
