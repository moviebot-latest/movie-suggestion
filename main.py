# ╔══════════════════════════════════════════════════════════════════════╗
# ║                  🎬 CineBot v5 — GEMINI 2.0 + NO-REPEAT RANDOM      ║
# ║                                                                      ║
# ║  ✅ V5 CHANGES:
# ║     • Gemini 2.0 Flash (fastest free model)
# ║     • No-repeat /random per user
# ║     • Async Gemini calls fixed
# ║
# ║  🐛 ALL BUGS FIXED:                                                 ║
...
ai_model  = None
ai_client = None  # for new google-genai SDK
...
@web_app.route("/")
def home(): return "🎬 CineBot v4 Running"

@web_app.route("/health")
def health(): return {"status": "ok", "version": "4.0"}
...
async def ai_ask(prompt):
    ...
    if ai_client:
        try:
            ...
            print(f"⚠️ New SDK error: {_e}")
    # Fallback: old SDK
    if ai_model:
        try:
            r = await asyncio.to_thread(ai_model.generate_content, prompt)
            return r.text
        except Exception as _e:
            print(f"⚠️ Old SDK error: {_e}")
    return None
...
#                   ✅ MOVIE CARD (FIXED)
# ═══════════════════════════════════════════════════════════════════
async def _send_movie_card(update, context, data, reply_to=None):
    ...
    temp_keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("⭐ Rate Movie",       callback_data="rate_tmp"),
         InlineKeyboardButton("🎥 Director Top 5",  callback_data=f"dir_{director.replace(' ', '_')}")],
    ])

    # ✅ FIXED: poster fail hone par text fallback
    try:
    ...
    # ✅ FIXED: now update buttons with real msg_id
    await sent.edit_reply_markup(reply_markup=InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🤖 AI Review",        callback_data=f"rev_{msg_id}"),
         InlineKeyboardButton("💡 Fun Facts",        callback_data=f"fun_{msg_id}")],
...
async def pick_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await animate_search(loader)
    data = get_omdb(imdb_id, by_id=True)
    try: await loader.delete()
    except: pass
...
async def review_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    review = await ai_movie_review(
        ...
        movie_data["plot"], movie_data["rating"]
    )
    try: await loader.delete()
    except: pass
    ...
    if review:
        ...
        await query.message.reply_text("❌ Review nahi mili. GEMINI_API check karo.")
...
async def funfact_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    loader = await query.message.reply_text("💡 Finding facts...\n" + progress_bar(0, 3))
    await animate_generic(loader, FRAMES["ai"])

    facts = await ai_fun_facts(
        movie_data["title"], movie_data["year"],
        movie_data["director"], movie_data["actors"]
    )
    try: await loader.delete()
    except: pass
    ...
    if facts:
        ...
        await query.message.reply_text("❌ Facts nahi mile. GEMINI_API check karo.")
...
async def adm_export_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_document(
        ...
        document=open("users_export.txt", "rb"),
...
print("✅ CineBot v4 Running — All Bugs Fixed + Advanced Features Active!")
