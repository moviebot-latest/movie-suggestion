...
try:
    ...
    print("⚠️ beautifulsoup4 not installed — DDG fallback disabled. pip install beautifulsoup4")
...
@web_app.route("/")
def home(): return "🎬 CineBot v10 Groq + Full Analysis Running"

@web_app.route("/health")
def health(): return {"status": "ok", "version": "10.0", "ai": "groq", "analysis": "full"}
...
#   Har 10 min mein self-ping karta hai taaki bot soye nahi
# ═══════════════════════════════════════════════════════════════════
def keep_alive_ping():
    ...
    if not RENDER_URL:
        ...
        return
    print(f"✅ Keep-alive started → {RENDER_URL}/health (every 10 min)")
    while True:
...
async def checkservers_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ai_text:
        text += f"\n\n🤖 *AI ANALYSIS:*\n{ai_text}"
    text = text[:4000]  # ✅ FIX: Telegram 4096 char limit
...
# ── Auto background checker — v10 ULTIMATE: Smart Domain Finder ──

# ════════════════════════════════════════════════════════════════
#  🔍 LAYER 1: AI Text Domain Extractor
#  Groq AI analysis mein already new domain hota hai — use karo
# ════════════════════════════════════════════════════════════════

def _extract_domains_from_ai_text(ai_text: str, site_key: str) -> list:
    """
    Groq ke diagnosis text se new domain URLs nikalo.
    AI text example:
      "Fix: Admin ko new domain (new6.hdhub4u.fo) update karna chahiye"
      "Fix: 123mkv.stream update karna chahiye"
    Returns: list of candidate full URLs with /?s=
    """
    if not ai_text:
        return []
    ...
    # Pattern 1: (domain.tld) — parentheses ke andar
    paren = re.findall(r'\(([a-z0-9][a-z0-9\-\.]+\.[a-z]{2,})\)', ai_text, re.IGNORECASE)
    # Pattern 2: https://domain.tld/...
    urls  = re.findall(r'https?://([a-z0-9\-\.]+\.[a-z]{2,})', ai_text, re.IGNORECASE)
    # Pattern 3: bare domain.tld in text
    bare  = re.findall(r'\b([a-z0-9][a-z0-9\-]*[a-z0-9]\.[a-z]{2,}(?:\.[a-z]{2,})?)\b', ai_text, re.IGNORECASE)

    skip_domains = {"google.com", "reddit.com", "telegram.org", "github.com",
                    "wikipedia.org", "youtube.com", "twitter.com", "facebook.com"}

    for raw in paren + urls + bare:
        raw = raw.lower().strip("./,;)")
        if raw in skip_domains:
            continue
        if not any(name in raw for name in base_names):
            continue
        if raw in seen:
            continue
        seen.add(raw)
        candidates.append(f"https://{raw}/?s=")
        print(f"   🎯 AI-extracted: {raw}")

    return candidates


# ════════════════════════════════════════════════════════════════
#  🔍 LAYER 2: DuckDuckGo Search (fallback jab AI text mein nahi mila)
# ════════════════════════════════════════════════════════════════

async def _ddg_search(query: str, max_results: int = 12) -> list:
    """DuckDuckGo HTML search — No API key needed."""
    if not _BS4_AVAILABLE:
        print("⚠️ bs4 not available — DDG search skipped")
        return []
    ...
    try:
        ...
        # Also extract bare URLs from raw HTML
        ...
        for u in urls:
            ...
            if u not in seen:
                seen.add(u)
                clean.append(u)
        print(f"   🔍 DDG '{query}' → {len(clean)} URLs")
        return clean[:max_results]
    except Exception as e:
        print(f"⚠️ DDG error: {e}")
        return []

def _extract_candidates_from_urls(all_urls: list, site_key: str, old_url: str) -> list:
    """Search result URLs se site-relevant domain candidates nikalo + TLD brute-force."""
    base_names = SITE_BASE_NAMES.get(site_key, [])
    ...
    for url in all_urls:
        try:
            ...
            if not domain or domain in seen:
                continue
            if not any(name in domain for name in base_names):
                continue
            seen.add(domain)
            candidates.append(f"https://www.{domain}/?s=")  # health check URL
        except Exception:
            continue

    # TLD brute-force fallback
    ...
    print(f"   📋 {len(candidates)} total candidates for {site_key}")
    return candidates

async def _ddg_find_candidates(site_key: str, old_url: str) -> list:
    """DDG search run karo aur candidates extract karo."""
    year    = now_ist().year
    ...
    for q in queries:
        ...
        await asyncio.sleep(2)  # DDG rate limit
    return _extract_candidates_from_urls(all_urls, site_key, old_url)


# ════════════════════════════════════════════════════════════════
#  ✅ URL Checker — single URL fast check
# ════════════════════════════════════════════════════════════════

async def _check_candidate_url(url: str) -> tuple:
    """Check karo URL UP hai? Returns (url, is_up, ms)"""
    try:
        timeout = aiohttp.ClientTimeout(sock_connect=7, sock_read=10, total=18)
        async with aiohttp.ClientSession(timeout=timeout) as sess:
            start = time.monotonic()
            async with sess.get(
                url, headers=_get_srv_headers(),
                allow_redirects=True, ssl=False,
            ) as resp:
                ms    = int((time.monotonic() - start) * 1000)
                is_up = resp.status in SRV_UP_CODES
                return url, is_up, ms
    except Exception:
        return url, False, 0

async def _find_first_working(candidates: list, batch_size: int = 8) -> tuple:
    """Candidates ko batches mein check karo, pehla UP wala return karo."""
    for i in range(0, len(candidates), batch_size):
        batch   = candidates[i: i + batch_size]
        results = await asyncio.gather(
            *[_check_candidate_url(u) for u in batch],
        ...
        for r in results:
            ...
            print(f"   {status} {urlparse(url).netloc} | {ms}ms")
            if is_up:
                return url, ms
...
# ════════════════════════════════════════════════════════════════
#  💾 Auto-Save + Admin Notify (shared by both layers)
# ════════════════════════════════════════════════════════════════

async def _autosave_new_domain(
    srv_key: str, srv_name: str,
    old_url: str, new_url: str, new_ms: int,
    method: str, bot, admin_id: int,
):
    """New domain auto-save karo aur admin ko notify karo with Undo button."""
    # Save to servers.json
    servers = load_json("servers", {k: v.copy() for k, v in DEFAULT_SERVERS.items()})
    servers[srv_key]["url"] = new_url
    ...
    log = load_failover_log()
    log.append(log_entry)
    save_failover_log(log)

    old_d  = urlparse(old_url).netloc or old_url[:40]
    new_d  = urlparse(new_url).netloc or new_url[:40]
    method_label = "🤖 AI Text Extract" if method == "ai_text" else "🔍 DDG Search"

    msg = (
        f"⚡ *AUTO DOMAIN UPDATE!* 🎉\n\n"
        f"*{srv_name}* ka naya domain auto-found & saved!\n\n"
        f"❌ Old (DOWN): `{old_d}`\n"
        f"✅ New (UP):  `{new_d}` | `{new_ms}ms`\n\n"
        f"🔧 Method: {method_label}\n"
        f"⏰ _{log_entry['timestamp']}_\n\n"
        f"Users ko ab working link milega! 🚀"
    )
    kb = [[
    ...
    try:
        ...
        print(f"⚠️ Autosave notify error: {e}")

    print(f"✅ AUTO-SAVED: {srv_name} → {new_d} ({new_ms}ms) [{method}]")


# ════════════════════════════════════════════════════════════════
#  🚀 MASTER: Smart Domain Finder — 2 Layer System
#  Layer 1: AI text se nikalo (instant, no extra API)
#  Layer 2: DDG search + TLD brute-force (fallback)
# ════════════════════════════════════════════════════════════════

async def smart_domain_finder(
    srv_key: str, srv_name: str,
    old_url: str, ai_text: str,
    bot, admin_id: int,
) -> bool:
    """
    2-Layer Smart Domain Finder:
    Layer 1 → AI text se domain extract → check → save
    Layer 2 → DDG search → TLD brute-force → check → save
    Returns True agar naya domain mila aur save ho gaya.
    """
    print(f"\n🤖 Smart Domain Finder: {srv_name}")

    # ── LAYER 1: AI text se extract ──────────────────────────────
    print(f"   [Layer 1] Extracting domains from AI diagnosis text...")
    l1_candidates = _extract_domains_from_ai_text(ai_text, srv_key)

    if l1_candidates:
        print(f"   [Layer 1] Checking {len(l1_candidates)} AI-suggested URLs...")
        new_url, new_ms = await _find_first_working(l1_candidates)
        if new_url:
            await _autosave_new_domain(
                srv_key, srv_name, old_url, new_url, new_ms, "ai_text", bot, admin_id
            )
            return True
        print(f"   [Layer 1] None working — falling to Layer 2")
    else:
        print(f"   [Layer 1] No domains in AI text — going to Layer 2")

    # ── LAYER 2: DDG Search + TLD brute-force ────────────────────
    print(f"   [Layer 2] DDG search for {srv_name}...")

    # Notify admin: searching...
    try:
        status_msg = await bot.send_message(
            chat_id=admin_id,
            text=(
                f"🔍 *Auto Domain Search* — *{srv_name}*\n\n"
                f"AI text mein domain nahi mila.\n"
                f"Internet pe search kar raha hoon...\n"
                f"⏳ _~30 seconds_"
            ),
            parse_mode="Markdown",
        )
    ...
    l2_candidates = await _ddg_find_candidates(srv_key, old_url)

    if not l2_candidates:
        print(f"   [Layer 2] No candidates found at all")
        fail_msg = (
            f"❌ *Domain Search Failed* — *{srv_name}*\n\n"
            f"Internet search bhi koi result nahi diya.\n"
            f"Manually naya URL Admin Panel se dalo.\n\n"
            f"Search hint: `{srv_name.lower()} new domain {now_ist().year}`"
        )
        if status_msg:
            try:
                await status_msg.edit_text(
                    ...
                    reply_markup=InlineKeyboardMarkup([[
                        InlineKeyboardButton("⚙️ Edit Server", callback_data="adm_servers")
                    ]])
                )
            except Exception:
                pass
    ...
    print(f"   [Layer 2] Checking {len(l2_candidates)} candidates...")
    new_url, new_ms = await _find_first_working(l2_candidates)

    if not new_url:
        fail_msg = (
            f"❌ *No Working Domain Found* — *{srv_name}*\n\n"
            f"{len(l2_candidates)} URLs check kiye — sab DOWN.\n"
            f"Site permanently band ho sakti hai.\n"
            f"Admin Panel se manually naya URL dalo."
        )
        if status_msg:
            try:
                await status_msg.edit_text(
    ...
    # Found! Delete status message, send success via _autosave_new_domain
    if status_msg:
        try:
    ...
    await _autosave_new_domain(
        srv_key, srv_name, old_url, new_url, new_ms, "ddg_search", bot, admin_id
    )
    return True


# ════════════════════════════════════════════════════════════════
#  ↩️ FAILOVER UNDO / CONFIRM CALLBACKS
# ════════════════════════════════════════════════════════════════

async def failover_undo_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """↩️ Undo — purana URL restore karo."""
    query = update.callback_query
    if not is_admin(query.from_user.id):
    ...
    if not entry:
        await query.edit_message_text("⚠️ Record nahi mila. Already undo ho gaya hoga.")
        return
    ...
    save_failover_log([e for e in log if e["id"] != log_id])

    old_d = urlparse(entry["old_url"]).netloc
    new_d = urlparse(entry["new_url"]).netloc
    await query.edit_message_text(
        f"↩️ *Undo Done!*\n\n"
        f"*{entry['srv_name']}* — Original URL wapas restore ho gaya.\n\n"
        f"✅ Restored: `{old_d}`\n"
        f"❌ Removed:  `{new_d}`\n\n"
        f"⚠️ _Original server abhi bhi DOWN ho sakta hai._\n"
        f"Manually naya URL Admin Panel se dalo.",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("⚙️ Edit Server", callback_data="adm_servers"),
...
async def failover_keep_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """✅ Confirm — new URL permanently rakho, log clean karo."""
    query = update.callback_query
    if not is_admin(query.from_user.id):
        await query.answer("❌ Admin only!", show_alert=True)
        return
    ...
    if entry:
        ...
        await query.edit_message_text(
            f"✅ *Confirmed!*\n\n"
            f"*{entry['srv_name']}* → `{new_d}` permanently saved.\n"
            f"_Undo option remove ho gaya._",
            parse_mode="Markdown",
        )
    else:
...
async def failoverlog_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/failoverlog — domain switch history dekho."""
    if not is_admin(update.effective_user.id):
    ...
    if not log:
        await update.message.reply_text(
            "📋 Koi domain switch history nahi.\nSab servers stable hain! ✅"
        )
        return
    ...
    for e in reversed(log[-10:]):
        ...
        method = "🤖 AI" if e.get("method") == "ai_text" else "🔍 DDG"
        lines.append(
            f"\n🔄 *{e['srv_name']}* [{method}]\n"
            f"   ❌ `{old_d}`\n"
            f"   ✅ `{new_d}` | {e['new_ms']}ms\n"
...
async def auto_server_checker(bot, admin_id: int):
    """
    v10 ULTIMATE: Smart Auto Domain Finder integrated
    DOWN detect → AI text se domain extract (Layer 1)
              → DDG search fallback (Layer 2)
              → Working domain auto-save
              → Admin notify with Undo/Confirm buttons
    """
    print("🕐 Auto server checker v10 ULTIMATE (Smart Domain Finder) started")
    await asyncio.sleep(300)

    _finder_attempted: set = set()  # Track: is cycle mein already try kiya
    ...
    while True:
        try:
            ...
            if down_keys:
                ...
                # ── AI Diagnosis (existing — used for both alert + domain extraction) ──
                ai_text = None
                if to_alert:
                    ai_text = await srv_ai_diagnose(results)
                ...
                if to_alert:
                    ...
                    if ai_text:
                        alert += f"\n\n🤖 *AI ANALYSIS:*\n{ai_text}"
                    alert += "\n\n⚡ _Smart Domain Finder running..._"
                    kb = [[
                        InlineKeyboardButton("🔄 Recheck", callback_data="srvchk_refresh"),
                        InlineKeyboardButton("⚙️ Edit",    callback_data="adm_servers"),
                    ]]
                    await bot.send_message(
                        chat_id=admin_id, text=alert[:4000],
                        parse_mode="Markdown",
                        reply_markup=InlineKeyboardMarkup(kb),
                    )
                    print(f"🚨 Alert: {len(to_alert)} DOWN")

                # ── SMART DOMAIN FINDER: 2+ consecutive fails ke baad ──
                for k in down_keys:
                    srv_name     = results[k].get("name", k)
                    old_url      = servers.get(k, {}).get("url", "")
                    consec_fails = results[k].get("consec_fails", 0)

                    if consec_fails < 2:
                        print(f"⏳ {srv_name}: {consec_fails} fail — wait for 2+")
                        continue
                    if k in _finder_attempted:
                        print(f"⏩ Domain finder already ran for {srv_name} — skip")
                        continue

                    _finder_attempted.add(k)
                ...
                _finder_attempted -= {k for k in _finder_attempted
            ...
            print(f"Auto checker v10 error: {e}")
            sleep_sec = SRV_CHECK_INTERVAL_HOURS * 3600
...
# ═══════════════════════════════════════════════════════════════════
#   END SERVER HEALTH CHECKER v10 ULTIMATE
# ═══════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════
#  🤖 AUTO DOMAIN FINDER CONFIG
#  Server DOWN → AI text se domain extract → DDG fallback → auto-save
# ══════════════════════════════════════════════════════════════════════

# Base keywords for each server (domain matching)
SITE_BASE_NAMES = {
    "s1": ["hdhub4u"],
    "s2": ["123mkv"],
    "s3": ["mkvcinemas"],
    "s4": ["worldfree4u"],
    "s5": ["bolly4u"],
    "s6": ["filmyfly", "1filmyfly", "filmyzilla"],
}

# DDG search queries per server (year placeholder = current year)
SITE_SEARCH_QUERIES = {
    "s1": ["hdhub4u new domain {year}", "hdhub4u working url {year}", "hdhub4u new link"],
    "s2": ["123mkv new domain {year}", "123mkv working url {year}", "123mkv new link"],
    ...
    "s6": ["1filmyfly new domain {year}", "filmyfly working url {year}", "filmyzilla new domain {year}"],
}

# TLD brute-force list
SITE_TLD_LIST = [
...
def save_failover_log(log: list):
    with _failover_log_lock:
...
def load_servers():
    # ✅ FIX: Har baar DEFAULT_SERVERS se fresh load — env vars se URLs aate hain
...
#
#  Har site ka search URL format alag hota hai:
#    /?s=MovieName           → Standard WordPress search
#    /search.html?q=Movie    → Custom search page (HdHub4u style)
#    /site-1.html?to-search= → FilmyFly style
#    /?s=                    → Just append title
#
#  Yeh function automatically sahi format mein URL banata hai.
#  Movie title: spaces → + (search engine friendly)
#               special chars → URL encoded
# ══════════════════════════════════════════════════════════════════════

def build_search_url(base_url: str, movie_title: str) -> str:
    """
    Smart URL builder — base URL ke format ko detect karke
    movie title correctly append karta hai.

    Handles:
      /?s=           → title URL-encoded, spaces as +
      /?s=           → same
      /search.html?q → same
      /site-1.html?to-search= → same
      Base URL only  → auto-appends /?s=title
      Already has ?  → direct append
    """
    # Clean title: trim whitespace
    title = movie_title.strip()

    # URL encode: spaces → + for cleaner search URLs
    encoded_plus  = title.replace(" ", "+")                    # "Pushpa 2" → "Pushpa+2"
    encoded_raw   = quote(title, safe="")                      # full percent-encode
    encoded_smart = quote(title, safe="").replace("%20", "+")  # hybrid: + for spaces

    # ── Detect format by URL ending ──

    # Format 1: ends with ?param= → directly append encoded title
    # e.g.  /?s=  |  ?q=  |  ?to-search=  |  ?query=  |  ?keyword=
    if re.search(r'\?[a-z\-_]+=\s*$', base_url, re.IGNORECASE):
        return base_url + encoded_smart

    # Format 2: Base URL ends with / (no query param at all)
    # e.g.  https://bolly4u.camera/
    # → Add /?s=title  (WordPress default)
    if base_url.endswith("/"):
        return base_url + f"?s={encoded_smart}"

    # Format 3: Base URL has no trailing slash and no query
    # e.g.  https://example.com
    if "?" not in base_url:
        return base_url + f"/?s={encoded_smart}"

    # Format 4: URL has ? already but not ending with =
    # e.g.  https://example.com/page?foo=bar
    # → Append &s=title
...
#
#  Search results page scrape karo → pehla matching movie link nikalo
#  Har site ka HTML structure alag hai — multiple selectors try karo
#  Fallback: agar direct link nahi mila → search URL use karo
#
#  Flow:
#    search_url → HTTP GET → BeautifulSoup parse
#    → Try selectors → Filter by title match → Return direct URL
#    → Timeout/error → Return search_url (fallback)
# ══════════════════════════════════════════════════════════════════════

# CSS selectors jo movie sites pe commonly use hote hain (priority order)
_MOVIE_LINK_SELECTORS = [
    # WordPress standard (most sites)
    "h2.entry-title a",
    "h3.entry-title a",
    "h1.entry-title a",
    ".entry-title a",
    ...
    ".post-title a",
    ".item-title a",
    # Custom selectors (specific sites)
    ".recent-movies .name a",
    ".gridlove-post-title a",
...
def _title_match_score(link_text: str, movie_title: str) -> int:
    """
    Movie title match score calculate karo (0-100).
    Year suffix ignore karta hai: 'RRR 2022' → 'RRR' se match.
    """
    if not link_text or not movie_title:
        return 0
    lt    = link_text.lower().strip()
    # Strip year from title for matching (e.g. "RRR 2022" → "RRR")
    mt    = re.sub(r'\b(19|20)\d{2}\b', '', movie_title.lower()).strip()
    words = [w for w in mt.split() if len(w) > 1]

    # Exact match (with or without year)
    if mt in lt or movie_title.lower() in lt:
        return 100
    # All meaningful words present
    if words and all(w in lt for w in words):
        return 85
    # More than half words match
...
async def _scrape_direct_link(
    ...
    """
    Search results page se direct movie link scrape karo.
    Returns: direct URL string, or None if not found.
    """
    if not _BS4_AVAILABLE:
        return None
    try:
        ...
        for selector in _MOVIE_LINK_SELECTORS:
            ...
            for tag in links[:8]:  # top 8 results only
                href = tag.get("href", "")
            ...
            if best_score >= 80:
                break  # Good match mila — stop trying more selectors
...
async def resolve_server_urls(
    search_urls: list,
    movie_title: str,
) -> list:
    """
    6 servers ke liye parallel mein direct links dhundo.
    Agar direct nahi mila → search URL wapas do (fallback).
    Returns: list of 6 final URLs (direct or search).
    """
    ...
    for i, (search_url, direct) in enumerate(zip(search_urls, direct_links)):
        if isinstance(direct, Exception) or not direct:
            final_urls.append(search_url)  # fallback
            print(f"   🔍 s{i+1}: fallback → search page")
        else:
            final_urls.append(direct)
            domain = urlparse(direct).netloc
            print(f"   🎯 s{i+1}: direct → {domain}")
    return final_urls
...
def progress_bar(current, total, length=10):
    if total == 0: return "[··········] 0%"  # ✅ FIX: division by zero
...
# ═══════════════════════════════════════════════════════════════════
#          GROQ AI — ORIGINAL FUNCTIONS (v10)
# ═══════════════════════════════════════════════════════════════════
...
async def _send_movie_card(update, context, data, reply_to=None, is_search=False):
    ...
    # ✅ Step 1: Smart search URLs (har site ke format ke hisab se)
    ...
    # ✅ Step 2: Start with search URLs immediately (no delay for user)
    #    Direct links resolve karega background mein — movie card turant dikhega
    urls = search_urls  # fallback (search pages) — turant show hoga
    ...
    # ✅ Background: direct links resolve karo (movie card turant show ho chuka)
    # Jab links resolve ho jayein → context.user_data update ho jaata hai silently
    async def _bg_resolve_links():
        try:
            print(f"🎯 [BG] Resolving direct links: {title}")
            direct = await resolve_server_urls(search_urls, title)
            # Update silently — user jab bhi "All 6 Servers" click karega, direct link milega
            if msg_id in context.user_data:
                context.user_data[msg_id]["servers"] = direct
                print(f"✅ [BG] Direct links updated for msg {msg_id}")
        except Exception as e:
...
async def leaderboard_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, u in enumerate(sorted_users):
        ...
        # ✅ FIX: Escape special Markdown chars in user name
...
def _upcom_init_db():
    # ✅ FIX: Use /tmp/movies.db — more reliable on Render free plan
...
async def upcoming_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Not a month/year — treat as movie name search (supports "Movie Name 2024")
        is_name_search = True

    # ── ✅ Movie name search (with optional year) ──
    if is_name_search:
        # Extract year from end if present e.g. "Spider-Man 2024"
        ...
        # ✅ Search with clean name (without year) for better TMDB results
        movies = await asyncio.to_thread(_upcom_search_by_name, search_q)

        # If year_hint given, filter by year
        if year_hint and movies:
            filtered = [m for m in movies if str(year_hint) in m.get("release", "")]
            if filtered:
                movies = filtered  # use filtered; else fallback to all results
...
async def adm_recv_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ✅ FIX: Fresh load karo file se, phir update karo
    servers = load_json("servers", {k: v.copy() for k, v in DEFAULT_SERVERS.items()})
    servers[sk]["url"]  = url
    servers[sk]["name"] = name
    save_json("servers", servers)
    bot_servers = servers  # global bhi sync karo
    try: await loader.delete()
    except: pass
    sent = await update.message.reply_text(
...
async def adm_send_alerts(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    - Har user ko uski saved movie alerts bhejo
    - Success ke baad alerts.json clear ho jata hai (fresh start)
    """
    query = update.callback_query
    ...
    if not alerts:
        sent = await query.message.reply_text(
            "📭 *Koi alerts saved nahi hain!*\n\n"
            "_Users ne abhi koi movie alert set nahi kiya._",
            parse_mode="Markdown"
        )
        asyncio.create_task(auto_delete(sent, 30))
        return
    ...
    for uid, movies in alerts.items():
        ...
        # Build one combined message per user (all their alerts together)
        movie_list = "\n".join(
    ...
    # ✅ Clear alerts after sending — fresh start
    save_json("alerts", {})
    ...
    result = await query.message.reply_text(
        ...
        f"_Alerts.json reset ho gaya — fresh start!_ 🔄",
        parse_mode="Markdown"
    )
    asyncio.create_task(auto_delete(result, 60))

# ✅ NEW: /sendalert — admin custom message to ALL users
async def sendalert_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/sendalert <message> — Send custom alert to ALL registered users"""
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("🔒 Sirf admins ke liye!", parse_mode="Markdown")
        return
    ...
    if not msg:
        await update.message.reply_text(
            ...
            "_Example:_ `/sendalert 🎬 KGF 3 aa gaya! Bot pe search karo!`",
            parse_mode="Markdown"
        )
        return
    users = load_json("users")
    loader = await update.message.reply_text(
        f"📢 *Sending to {len(users)} users...*", parse_mode="Markdown"
    )
    success = failed = 0
    for uid in list(users.keys()):
        ...
        try:
            await context.bot.send_message(
                ...
                text=(
                    ...
                    f"{msg}\n\n"
                    f"━━━━━━━━━━━━━━━━━━\n"
                    f"_CineBot — Your Movie Assistant_ 🎬"
                ),
                parse_mode="Markdown"
            )
            success += 1
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "📢 /sendalert    — Alert all users (Admin) ✅NEW\n"
...
#                        BOT START
# ═══════════════════════════════════════════════════════════════════
# ✅ FIX: Daily reminder loop — har din 9 AM IST pe fire hota hai
async def _reminder_daily_loop(bot):
    while True:
        try:
            ...
            await asyncio.sleep(3600)  # retry in 1hr on error
...
async def post_init(application):
    ...
    # ✅ FIX: Upcoming reminders daily checker — har din 9 AM IST
...
application.add_handler(CommandHandler("sendalert",   sendalert_cmd))   # ✅ NEW: admin alert to all users
...
# ✅ v10 Smart Domain Finder callbacks
...
print("✅ CineBot v10 — Groq AI + Full Analysis + Server Health Checker v10 ULTIMATE (Smart Domain Finder) integrated!")
