# ╔══════════════════════════════════════════════════════════════════════════╗
# ║         🎬  CineBot v9 — GROQ EDITION (Gemini → Groq API)             ║
# ║                                                                          ║
# ║  ✅ V9 CHANGES:                                                         ║
# ║     • All v8 bugs fixed                                                  ║
...
# ║     • Version string v9 fixed everywhere                                 ║
# ║                                                                          ║
# ║  ✅ GROQ EDITION:                                                        ║
# ║     • Gemini API completely removed                                      ║
# ║     • Groq API (llama-3.3-70b-versatile) integrated                     ║
# ║     • Sync requests → async aiohttp for Groq calls                      ║
...
# ║                                                                          ║
# ║  APIs needed:                                                            ║
# ║     BOT_TOKEN, OMDB_API, TMDB_API, GROQ_API, ADMIN_ID                  ║
# ╚══════════════════════════════════════════════════════════════════════════╝
...
@web_app.route("/")
def home(): return "🎬 CineBot v9 Groq Edition Running"

@web_app.route("/health")
def health(): return {"status": "ok", "version": "9.0", "ai": "groq"}
...
# ═══════════════════════════════════════════════════════════════════
#                   GROQ AI FUNCTIONS  ✅ (replaces Gemini)
# ═══════════════════════════════════════════════════════════════════
async def ai_ask(prompt: str) -> Optional[str]:
...
async def ai_fix_movie_name(raw_name: str) -> str:
    """Fix misspelled/Hinglish movie names."""
    if not GROQ_API:
        return raw_name
    result = await ai_ask(
...
# /movieinfo command — uses movie_info module (TMDB-rich)
...
async def compare_recv2(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if result:
        ...
        await update.message.reply_text(
            "❌ Comparison nahi hua. GROQ_API check karo.",
            parse_mode="Markdown"
        )
    return ConversationHandler.END
...
async def addadmin_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) >= 2:
        try:
            ...
            try:
                await context.bot.send_message(chat_id=target_id, text=(
                    f"🎉 *Congratulations!*\n\nAapko *CineBot* ka *Temporary Admin* banaya gaya hai! 🚀\n\n"
        ...
        try:
            await context.bot.send_message(chat_id=target_id, text=(
                f"🎉 *Congratulations!*\n\nAapko *CineBot* ka *Permanent Admin* banaya gaya hai! 🚀\n\n🔑 *Admin Panel:* /admin"
            ), parse_mode="Markdown")
        except Exception: pass
...
async def help_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "╔═══════════════════╗\n║  ℹ️  *CINEBOT HELP*  ║\n╚═══════════════════╝\n\n"
        "🔎 *Movie Search:*\n_Seedha movie naam type karo_\n_Groq AI galat naam bhi fix karega!_\n\n"
...
print("✅ CineBot v9 Groq Edition — Gemini removed, Groq (Llama 3.3) integrated!")
